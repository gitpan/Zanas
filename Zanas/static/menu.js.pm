...
function addItem(idItem, text, hint, location, id) {
	...
	if (n)
	{
		...
		MENUitem += "<a id='" + id + "' href='" + location + "' class=lnk0 onmouseover=\"displaySubMenu('"+idItem+"')\">";
		MENUitem += "|&nbsp;";
		MENUitem += text;
		MENUitem += "</a>";
		MENUitem += "</ilayer>";
...
//	this.bgColor     = menucolor;
	this.menuFont = "bold 11px Verdana"; 
	...
	this.subMenuPaneWidth = 120; //submenuwidth;
	...
	HTMLstr += "\n";
	if (ie||ns6) HTMLstr += "<div id='MainTable' ssstyle='position:absolute;top:0;left:0;'>\n";
	HTMLstr += "<table width='100%' class=bgr8 border='"+this.mainPaneBorder+"'>\n";
	HTMLstr += "<tr>";
	if (n) HTMLstr += "<td>&nbsp;";

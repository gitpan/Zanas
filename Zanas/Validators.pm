...
sub vld_inn_10 {
	...
	$_REQUEST {$name} =~ /^\d{10}$/ or return "#$name#:Код ИНН должен состоять из 10 арабских цифр";
	...
	$checksum == 0 + substr ($_REQUEST {$name}, -1, 1) or return "#$name#:Не сходится контрольная сумма ИНН";
...
sub vld_okpo {
	...
	$_REQUEST {$name} =~ /^\d{8}$/ or return "#$name#:Код ОКПО должен состоять из 8 арабских цифр";
	...
	if ($checksum_1 > 9) {
		$checksum_2 == 0 + substr ($_REQUEST {$name}, -1, 1) or return "#$name#:Не сходится контрольная сумма ОКПО";
	}
	else {
		$checksum_1 == 0 + substr ($_REQUEST {$name}, -1, 1) or return "#$name#:Не сходится контрольная сумма ОКПО";
	}
...
sub vld_ogrn {
	...
	$_REQUEST {$name} =~ /^\d{13}$/ or return "#$name#:Код ОГРН должен состоять из 13 арабских цифр";
	$_REQUEST {$name} =~ /^[12]/ or return "#$name#:1-я цифра ОГРН может быть только 1 (основной номер) или 2 (иной номер)";
	(0 + substr ($_REQUEST {_ogrn}, -1, 1) == (substr ($_REQUEST {$name}, 0, 12)) % 11) or return "#$name#:Не сходится контрольная сумма ОГРН";
